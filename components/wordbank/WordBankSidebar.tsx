...
export function WordBankSidebar({ totalCount, dueCount, weakCount, masteredCount, topics, recentStories, activeFilter }: WordBankSidebarProps) {
  return (
    <aside className="w-[200px] shrink-0 hidden lg:block">
      <nav className="space-y-1 mb-6">
        <CategoryLink href="/wordbank" label="All topics" count={totalCount} active={!activeFilter} />
        <CategoryLink href="/wordbank?filter=due" label="Due for review" count={dueCount} active={activeFilter === "due"} badgeColor={dueCount > 0 ? "amber" : undefined} />
        <CategoryLink href="/wordbank?filter=weak" label="Weak words" count={weakCount} active={activeFilter === "weak"} badgeColor={weakCount > 0 ? "rose" : undefined} />
        <CategoryLink href="/wordbank?filter=mastered" label="Mastered" count={masteredCount} active={activeFilter === "mastered"} />
      </nav>
