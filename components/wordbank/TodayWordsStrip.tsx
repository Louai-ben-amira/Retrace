...
export function TodayWordsStrip({ words, nativeLanguage = "ar" }: { words: WordBankEntry[]; nativeLanguage?: string }) {
  ...
  return (
    <div className="mb-8">
      <h2 className="text-sm font-semibold text-cream/50 uppercase tracking-wide mb-3">Today&apos;s words</h2>
      <div className="flex gap-2.5 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {words.map((w) => {
          const due = w.dueDate <= now;
          return (
            <div
              key={w.id}
