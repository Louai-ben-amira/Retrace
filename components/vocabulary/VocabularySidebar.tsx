...
// Mirrors WordBankSidebar's mobile treatment — below lg the vertical sidebar is replaced
// by a scrolling chip row so the SRS filters stay reachable on a phone.
function FilterChip({
  href, label, count, active, badgeColor,
}: { href: string; label: string; count: number; active: boolean; badgeColor?: "amber" | "rose" }) {
  return (
...
export function VocabularySidebar({ totalCount, dueCount, weakCount, masteredCount, topics, recentStories, activeFilter, uiLanguage }: VocabularySidebarProps) {
  ...
  return (
    ...
    <div className="lg:hidden -mx-4 px-4 overflow-x-auto no-scrollbar">
      <div className="flex items-center gap-2 pb-1 w-max">
        <FilterChip href="/vocabulary/mine" label={t.wordbank.allTopics} count={totalCount} active={!activeFilter} />
        <FilterChip href="/vocabulary/mine?filter=due" label={t.wordbank.dueForReview} count={dueCount} active={activeFilter === "due"} badgeColor="amber" />
        <FilterChip href="/vocabulary/mine?filter=weak" label={t.wordbank.weakWords} count={weakCount} active={activeFilter === "weak"} badgeColor="rose" />
        <FilterChip href="/vocabulary/mine?filter=mastered" label={t.wordbank.masteredFilter} count={masteredCount} active={activeFilter === "mastered"} />
        {topics.map((topic) => {
          const meta = topicMeta(topic.key);
          return (
            <Link
              key={topic.key}
              href={`/vocabulary/mine/topic/${topic.key}`}
    ...
    <aside className="w-[200px] shrink-0 hidden lg:block">
      <nav className="space-y-1 mb-6">
        <CategoryLink href="/vocabulary/mine" label={t.wordbank.allTopics} count={totalCount} active={!activeFilter} />
        <CategoryLink href="/vocabulary/mine?filter=due" label={t.wordbank.dueForReview} count={dueCount} active={activeFilter === "due"} badgeColor={dueCount > 0 ? "amber" : undefined} />
        <CategoryLink href="/vocabulary/mine?filter=weak" label={t.wordbank.weakWords} count={weakCount} active={activeFilter === "weak"} badgeColor={weakCount > 0 ? "rose" : undefined} />
        <CategoryLink href="/vocabulary/mine?filter=mastered" label={t.wordbank.masteredFilter} count={masteredCount} active={activeFilter === "mastered"} />
      </nav>
