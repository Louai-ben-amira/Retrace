...
export default async function VocabularyStoryPage({ params }: StoryPageProps) {
  ...
  return (
    <div>
      <Link href="/vocabulary/mine" className="text-sm text-cream/40 hover:text-cream/70">← All topics</Link>
      <h1 className="font-serif text-[28px] font-bold tracking-tight text-cream mt-2 mb-1">{story.title}</h1>
      <p
      ...
      <p className="text-cream/50 mb-8">
        {words.length} word{words.length === 1 ? "" : "s"} learned from this story ·{" "}
        <Link href={`/story/${story.id}`} className="text-brand-400 hover:underline">Read it again →</Link>
      </p>

      <VocabularyMasteryBar counts={masteryCounts} total={words.length} />

      <div className="grid sm:grid-cols-2 gap-3">
        {words.map((w) => <VocabularyWordCard key={w.id} word={w} showStory={false} nativeLanguage={user.nativeLanguage} />)}
      </div>
    </div>
  );
}
